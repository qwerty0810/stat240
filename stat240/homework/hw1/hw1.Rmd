...
```{r}
mendota = read_csv("lake-mendota-winters-2021.csv")
```
...
What are the standard deviations of `r` and `y`?  19.64127, 19.64127
...
ggplot(cars, aes(x = speed, y = dist)) +
  ...
  labs(title = "Cars data from the 1920s") +
  xlab("Speed (mph)") +
...
monona = drop_na(read_csv("lake-monona-winters-2021.csv"))
```
...
ggplot(monona, aes(x = duration)) +
  ...
  labs(title = "Distribution of Days Lake Monona is Frozen",
       x = "Duration (days)",
       y = "Frequency")

num_winters <- sum(monona$duration >= 80 & monona$duration <= 100)
...
ggplot(monona, aes(x = year1, y = duration)) +
  ...
  geom_hline(yintercept = mean(monona$duration), color = "red", linetype = "dashed") +
  labs(title = "Trends in Lake Monona Winter Duration",
       x = "Year",
       y = "Duration (days)")
```
...
Yes, the line differs from the straight line in a important manner. The important difference between the curve and the straight line indicates that the relationship between year1 and duration is not strictly linear.The curve suggests that the duration of winter freeze on Lake Monona may have some variability and fluctuations over the years, rather than a consistent linear decrease. Therefore, it's important to consider the non-linear trend captured by the curve rather than relying solely on the straight line for interpreting the relationship between the variables.
